...
float sensor_amp_t = 0.0; // temp sensor amp measurement
...
void setup() //   <----------------------------------------------------------------------------------------<<<<<<<<<<<<<<<<<<<
{
  ...
  screenshot_switch = boolean(screenshot_switch_xml.getContent());
  XML create_end_data_table_xml = config.getChild("seismograph_config/create_end_data_table");
  create_end_data_table = boolean(create_end_data_table_xml.getContent());
  ...
  proximity = boolean(proximity_xml.getContent());

  //Start point 
  Y[0] = y_ref;
  X[0] = x_ref;

  amp[0] = 0.0; // Starting Amplitude = 0 
  ...
  sensor1 = 0;
  sensor_amp_max = 0.0;
  sensor_amp_t = 0.0; // temp sensor amp measurement
      ...
      if (proximity == true && sensors.length >= 3) 
      {
        ...
        if(avg_times > 0)
        {
          ...
          if(avg_times == 1)
          {
           ...
           sensor0_d = int(avg0/s0.length);
           sensor1_d = int(avg1/s0.length);
          }
  ...
  else if (osc_button == false) //onn mean
  {
    if (osc_toggle == true)
    {
      ...
      { 
        ...
        Y = append(Y, y_ref -int(map(amp[amp.length-1] * amp_mag_fac, 0, max_cons_amp, 0, height/2)));//int((amp[amp.length-1]) * amp_mag_fac));  // PREVIOUSLY---> Y = append(Y, y_ref -int((amp_time[amp_time.length-1]) * amp_mag_fac));
        X = append(X, x_ref);
...
void createDataTable() // creates a table from the data of arrays and saves it to a csv file with date-time name as passed in the arguments
{
  for (int i = 0; i < X.length; i++)
  {
